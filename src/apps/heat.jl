...
Δp -- перепад давления
"""
function app_heater_W(ms_in::MaterialStream, W::EnergyStream, Δp::Float64 = 0.0) #W поток в ваттах
	h_in = mstream_H_T(ms_in, ms_in.T)    #начальная удельная энтальпия
    h_k = h_in + W.Q / ms_in.N	#посчитать конечную энтальпия
    ...
    p_k = ms_in.p
    return mstream_TpA(ms_in.N, T_k, p_k, A, ms_in.model)
end
